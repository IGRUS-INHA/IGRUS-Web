...
|-------|----------|------|--------|--------|
| Phase 1: 버그 수정 | 2 | 2 | 0 | 100% |
| Phase 2: Admin 조회 API | 5 | 5 | 0 | 100% |
| Phase 3: 영구 보관 정책 | 2 | 2 | 0 | 100% |
| Phase 4: 테스트 | 2 | 2 | 0 | 100% |
| Phase 5: 문서 | 1 | 1 | 0 | 100% |
| **Total** | **12** | **12** | **0** | **100%** |
...
- [x] **[T1]** `RecordFailedAttemptService.recordFailedAttempt()`에 `@Transactional(propagation = Propagation.REQUIRES_NEW)` 적용
  - 파일: `security/auth/common/service/login/RecordFailedAttemptService.java`
  - 별도 트랜잭션으로 실행되어 호출자 트랜잭션 롤백 시에도 카운트 유지

- [x] **[T2]** `RecordLoginFailureService`의 두 `recordFailure()` 오버로드 메서드에 `@Transactional(propagation = Propagation.REQUIRES_NEW)` 적용
...
- [x] **[T3]** `LoginHistoryResponse` DTO 생성
...
- [x] **[T4]** `LoginHistoryRepository`에 복합 필터 쿼리 추가
...
- [x] **[T5]** `GetLoginHistoryForAdminService` 서비스 생성
...
- [x] **[T6]** `AdminLoginHistoryController` 생성
...
- [x] **[T7]** `ApiSecurityConfig`에 admin 경로 추가
...
- [x] **[T8]** `LoginHistoryRepository`에서 `deleteByAttemptedAtBefore` 메서드 제거
  - 파일: `security/auth/common/repository/LoginHistoryRepository.java`
...
- [x] **[T10]** `AdminLoginHistoryControllerTest` 통합 테스트 작성
...
- [x] **[T11]** `GetLoginHistoryForAdminServiceTest` 통합 테스트 작성
