...
- **테스트 범위**: 댓글 CRUD HTTP 레이어 통합 테스트
- **구현된 테스트 케이스**:
  - `CreateCommentTest` 내부 클래스: CMT-001, CMT-002, CMT-003, CMT-004, CMT-006, CMT-007, 권한 없는 사용자(준회원) 접근 거부
  - `CreateReplyTest` 내부 클래스: CMT-010, CMT-011, CMT-014, 존재하지 않는 댓글에 대댓글 시도
  - `GetCommentsTest` 내부 클래스: CMT-020, CMT-021, CMT-023, CMT-025
  - `DeleteCommentTest` 내부 클래스: CMT-030, CMT-033, CMT-034, CMT-035, CMT-036
  - `AuthenticationTest` 내부 클래스: 비인증 사용자 접근 거부, 준회원 접근 거부
...
- **구현된 테스트 케이스**:
  - `LikeCommentTest` 내부 클래스: CMT-040, CMT-042, 중복 좋아요 방지, 존재하지 않는 댓글 좋아요 시도, 준회원 접근 거부
  - `UnlikeCommentTest` 내부 클래스: CMT-041, 좋아요하지 않은 댓글 취소 시도, 존재하지 않는 댓글 좋아요 취소 시도, 준회원 접근 거부
...
| 1.3 | 2026-01-28 | - | Controller 통합 테스트 구현 완료 (CommentControllerTest, CommentLikeControllerTest), 테스트 케이스 상태 업데이트 (22개 추가 완료) |
