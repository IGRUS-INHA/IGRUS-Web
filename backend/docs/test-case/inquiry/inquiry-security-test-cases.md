...
**현재 상태**: InquiryControllerSecurityTest로 대부분의 테스트 케이스가 구현됨 (GAP-INQ-04 해소).

**테스트 인프라**:
- `ServiceIntegrationTestBase` + `@AutoConfigureMockMvc` (MockMvc)
- 역할별 인증 토큰 생성 (`UsernamePasswordAuthenticationToken` + `AuthenticatedUser`)
...
### 4.1 컨트롤러 테스트
- **파일**: `backend/src/test/java/igrus/web/inquiry/controller/InquiryControllerSecurityTest.java`
- **테스트 범위**: INQ-SEC-001~002, 010~015, 020~027, 030~038, 040~044, 051~052, 054~055
- **테스트 수**: 42개
- **GAP-INQ-04 해소**: 컨트롤러 레벨 RBAC 검증 테스트 전체 구현
...
### 4.3 미구현 테스트
- INQ-SEC-050 (BCrypt 해싱 확인): INQ-G-007에서 서비스 레벨로 간접 검증됨
- INQ-SEC-053 (soft delete 접근 불가): INQ-A-072~073에서 서비스 레벨로 간접 검증됨
...
| 1.1 | 2026-02-11 | - | InquiryControllerSecurityTest 구현, GAP-INQ-04 해소, INQ-SEC-041 예상 결과를 403으로 수정 |
