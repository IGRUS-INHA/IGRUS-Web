...
import igrus.web.generated.model.ApiBoardDetailResponse;
import igrus.web.generated.model.ApiBoardListResponse;
...
public class BoardController implements BoardApi {
    ...
    public ResponseEntity<List<ApiBoardListResponse>> getBoardList() {
        ...
        var boards = getBoardListService.getBoardList(role);

        List<ApiBoardListResponse> response = boards.stream()
                .map(board -> new ApiBoardListResponse()
                        .code(board.code())
                        .name(board.name())
    ...
    @Override
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiBoardDetailResponse> getBoardByCode(String code) {
        AuthenticatedUser user = SecurityUtils.requireCurrentUser();
        log.info("게시판 상세 조회 요청 - code: {}, userId: {}, role: {}", code, user.userId(), user.role());

        UserRole role = EnumUtils.fromStringOrNull(UserRole.class, user.role());
        var board = getBoardByCodeService.getBoardByCode(code, role);

        ApiBoardDetailResponse response = new ApiBoardDetailResponse()
                .code(board.code())
                .name(board.name())
                .description(board.description())
