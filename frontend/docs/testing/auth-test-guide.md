...
## 회원가입 플로우 변경 (2026-07-20)

`/signup` 페이지가 아래와 같이 변경되었습니다. 이후 시나리오 중 스텝(다단계) 기반 설명은 구버전 기준이므로 참고용으로만 사용하세요.
...
- [ ] "가입 유형 다시 선택"으로 돌아간 뒤 유형을 바꿔도 폼 값이 올바르게 초기화됨
...
### 시나리오 3: 스텝 이동 시 중복 차단 (구버전 — 2026-07-20 단일 화면 폼 전환으로 폐기)
