...
export class FooterComponent {
  ...
  constructor(page: Page) {
    ...
    // Legal links
    this.privacyPolicyLink = this.footer.getByRole("link", {
      name: "개인정보처리방침",
    });
    this.termsOfServiceLink = this.footer.getByRole("link", {
      name: "이용약관",
    });
  ...
  async verifyDarkMode(isDark: boolean) {
    ...
    if (isDark) {
      ...
      return (
        bgColor.includes("249, 250, 251") || bgColor.includes("255, 255, 255")
      );
    }
  }
}
...
test.describe("Footer internal navigation", () => {
  ...
  test("should navigate to board page when clicking board link", async ({
    page,
  }) => {
    const footer = new FooterComponent(page);
  ...
  test("should navigate to events page when clicking events link", async ({
    page,
  }) => {
    const footer = new FooterComponent(page);
  ...
  test("should navigate to inquiry page when clicking inquiry link", async ({
    page,
  }) => {
    const footer = new FooterComponent(page);
...
test.describe("Footer dark mode support", () => {
  ...
  test("should apply light mode styles when theme is light", async ({
    page,
  }) => {
    await page.goto("/");
