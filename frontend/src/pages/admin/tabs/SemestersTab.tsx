...
export default function SemestersTab() {
  ...
  const { mutate: registerMembers, isPending: registering } = useRegisterMembers({
    mutation: {
      onSuccess: (res) => {
        const d = res.status === 200 ? res.data : undefined;
        addToast({ type: 'success', title: '등록 완료', message: `${d?.registeredCount ?? 0}명 등록됨` });
        setSelected([]);
        queryClient.invalidateQueries({ queryKey: [`/api/v1/admin/semesters/${year}/${semester}/candidates`] });
      },
      onError: () => { addToast({ type: 'error', message: '등록 실패' }); },
    },
  });
  ...
  const { mutate: removeMembers, isPending: removing } = useRemoveMembers({
    mutation: {
      onSuccess: (res) => {
        ...
        addToast({ type: 'success', title: '제외 완료', message: `${count}명 제외됨` });
        setSelected([]);
        queryClient.invalidateQueries({ queryKey: [`/api/v1/admin/semesters/${year}/${semester}/candidates`] });
      },
      onError: () => { addToast({ type: 'error', message: '제외 실패' }); },
    },
  });
    ...
    <div className="space-y-s6">
      ...
      <Card className="p-s5">
        <div className="flex gap-s4 items-center">
          ...
          <div className="flex gap-s2 items-center">
            ...
            <select
              value={semester}
              onChange={(e) => { setSemester(Number(e.target.value)); setSelected([]); }}
              className="px-s3 py-s2 rounded-r2 border border-border bg-background text-sm"
            >
      ...
      {candidates.length > 0 && (
        <div className="flex gap-s3 items-center">
          ...
          <Button
            size="sm"
            onClick={() => registerMembers({ year, semester, data: { userIds: selected } })}
      ...
      <Card className="p-s5 overflow-x-auto">
        <table className="w-full text-left">
          <thead>
            <tr className="text-c1 text-muted-foreground uppercase tracking-widest border-b border-border">
              ...
              <th className="pb-s4 font-bold hidden lg:table-cell">등록 여부</th>
            </tr>
          </thead>
        ...
        {candidates.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">등록 후보 회원이 없습니다.</div>
        )}
      </Card>
    </div>
  );
