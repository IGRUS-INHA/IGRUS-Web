...
const STATUS_FILTER_OPTIONS: { value: GetAllInquiriesStatus | ''; label: string }[] = [
  { value: '', label: '전체 상태' },
...
export default function InquiriesTab() {
  ...
  const { mutate: updateStatus } = useUpdateInquiryStatus({
    mutation: {
      onSuccess: () => { addToast({ type: 'success', message: '상태 변경 완료' }); invalidate(); },
      onError: () => { addToast({ type: 'error', message: '상태 변경 실패' }); },
    },
  });
  ...
  const { mutate: createReply, isPending: replying } = useCreateReply({
    mutation: {
      onSuccess: () => {
        ...
        setReplyContent('');
        invalidate();
        if (selectedId) queryClient.invalidateQueries({ queryKey: [`/api/v1/inquiries/${selectedId}`] });
      },
      onError: () => { addToast({ type: 'error', message: '답변 작성 실패' }); },
    },
  });
  ...
  const { mutate: updateReply, isPending: updatingReply } = useUpdateReply({
    mutation: {
      ...
      onError: () => { addToast({ type: 'error', message: '답변 수정 실패' }); },
    },
  });
  ...
  const { mutate: createMemo, isPending: creatingMemo } = useCreateMemo({
    mutation: {
      ...
      onError: () => { addToast({ type: 'error', message: '메모 작성 실패' }); },
    },
  });
  ...
  const { mutate: deleteInquiry } = useDeleteInquiry({
    mutation: {
      ...
      onError: () => { addToast({ type: 'error', message: '문의 삭제 실패' }); },
    },
  });
  ...
  return (
    <div className="space-y-s6">
      ...
      <div className="flex gap-s2">
        <select
          value={typeFilter}
          onChange={(e) => { setTypeFilter(e.target.value as GetAllInquiriesType | ''); setPage(1); }}
        ...
        <select
          value={statusFilter}
          onChange={(e) => { setStatusFilter(e.target.value as GetAllInquiriesStatus | ''); setPage(1); }}
          className="px-s3 py-s2 rounded-r2 border border-border bg-background text-sm"
        >
        ...
        {selectedId && detail && (
          <Card className="w-[400px] p-s5 space-y-s5 flex-shrink-0 max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-start">
              ...
              <button type="button" onClick={() => setSelectedId(null)} className="text-muted-foreground hover:text-foreground cursor-pointer">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="text-sm text-muted-foreground space-y-s1">
              ...
              <p>작성자: {detail.authorName} ({detail.authorEmail})</p>
              <p>작성일: {detail.createdAt ? new Date(detail.createdAt).toLocaleString('ko-KR') : '-'}</p>
            </div>
            ...
            <div className="flex gap-s2 items-center">
              ...
              <select
                value={detail.status ?? ''}
                onChange={(e) => updateStatus({ id: selectedId, data: { status: e.target.value as UpdateInquiryStatusRequestStatus } })}
              ...
              {detail.reply ? (
                <div className="space-y-s2">
                  ...
                  <textarea
                    ...
                    onBlur={(e) => {
                      if (e.target.value && e.target.value !== detail.reply?.content) {
                        updateReply({ id: selectedId, data: { content: e.target.value } });
                      }
                    }}
                ...
                <div className="space-y-s2">
                  ...
                  <Button
                    size="sm"
                    onClick={() => createReply({ id: selectedId, data: { content: replyContent } })}
            ...
            <div className="space-y-s3">
              ...
              <div className="flex gap-s2">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => createMemo({ id: selectedId, data: { content: memoContent } })}
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                if (confirm('정말 삭제하시겠습니까?')) deleteInquiry({ id: selectedId });
              }}
            >
              문의 삭제
            </Button>
