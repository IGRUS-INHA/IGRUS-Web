...
import { useGetUserDetail, useEditUserInfo } from '@/api/model/admin-user-management/admin-user-management';
...
function buildPatchPayload(
  ...
  if (edited.studentId !== original.studentId) payload.studentId = edited.studentId;
  if (edited.email !== original.email) payload.email = edited.email;
  if (edited.name !== original.name) payload.name = edited.name;
  if (edited.phoneNumber !== original.phoneNumber) payload.phoneNumber = edited.phoneNumber;
  if (edited.department !== original.department) payload.department = edited.department;
  if (edited.grade !== original.grade) payload.grade = edited.grade;
  if (edited.enrollmentStatus !== original.enrollmentStatus) payload.enrollmentStatus = edited.enrollmentStatus as NonNullable<AdminEditUserInfoRequest['enrollmentStatus']>;
  if (edited.gender !== original.gender) payload.gender = edited.gender as NonNullable<AdminEditUserInfoRequest['gender']>;
  return payload;
}
...
export default function UserEditModal({ userId, onClose }: UserEditModalProps) {
  ...
  useEffect(() => {
    if (detail) {
      reset({
        ...
        enrollmentStatus: (detail.enrollmentStatus as UserEditFormData['enrollmentStatus']) ?? 'ENROLLED',
  ...
  const { mutate: editUserInfo, isPending } = useEditUserInfo({
    mutation: {
      onSuccess: () => {
        addToast({ type: 'success', title: '정보 수정 완료', message: '회원 정보가 수정되었습니다.' });
      ...
      onError: (error: unknown) => {
        if (hasErrorCode(error, 'DUPLICATE_STUDENT_ID')) {
          ...
          setError('phoneNumber', { message: '이미 사용 중인 전화번호입니다.' });
        } else {
          addToast({ type: 'error', title: '정보 수정 실패', message: getErrorMessage(error) });
        }
      },
    },
  });
  ...
  const renderContent = () => {
    if (isLoading) {
      return <div className="flex items-center justify-center py-12 text-muted-foreground">로딩 중...</div>;
    }

    if (!detail) {
      return <div className="flex items-center justify-center py-12 text-muted-foreground">사용자 정보를 불러올 수 없습니다.</div>;
    }
    ...
    return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-s4">
            ...
            <FormField label="이메일" error={errors.email?.message}>
              <Input {...register('email')} type="email" placeholder="user@inha.edu" />
            </FormField>
              ...
              <div className="flex gap-s2">
                {ENROLLMENT_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setValue('enrollmentStatus', opt.value, { shouldValidate: true })}
                    className={cn(
                      'flex-1 py-s2 rounded-r2 text-sm font-medium border transition-colors cursor-pointer',
