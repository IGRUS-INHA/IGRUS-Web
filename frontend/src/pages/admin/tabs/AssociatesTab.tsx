...
export default function AssociatesTab() {
  ...
  const invalidate = () => {
    queryClient.invalidateQueries({
      queryKey: ["/api/v1/admin/associates/pending"],
    });
    queryClient.invalidateQueries({
      queryKey: ["/api/v1/admin/associates/rejected"],
    });
  ...
  const { data: rejectedRes, isLoading: rejectedLoading } =
    useGetRejectedAssociates(
      { page: page - 1, size: 20 },
  ...
  const { mutate: approveOne, isPending: approvingOne } = useApproveAssociate({
    mutation: {
      onSuccess: () => {
        addToast({ type: "success", message: "승인 완료" });
        invalidate();
      },
      onError: () => {
        addToast({ type: "error", message: "승인 실패" });
      },
    },
  });
  ...
  const { mutate: rejectOne, isPending: rejectingOne } = useRejectAssociate({
    mutation: {
      onSuccess: () => {
        addToast({ type: "success", message: "거절 완료" });
        setRejectingId(null);
        setRejectReason("");
        invalidate();
      },
      onError: () => {
        addToast({ type: "error", message: "거절 실패" });
      },
    },
  });
  ...
  const { mutate: approveBulk, isPending: approvingBulk } = useApproveBulk({
    mutation: {
      onSuccess: (res) => {
        ...
        addToast({
          type: "success",
          message: `${d?.approvedCount ?? 0}명 일괄 승인 완료`,
        });
        setSelected([]);
        invalidate();
      },
      onError: () => {
        addToast({ type: "error", message: "일괄 승인 실패" });
      },
    },
  });
  ...
  const { mutate: rejectBulk, isPending: rejectingBulk } = useRejectBulk({
    mutation: {
      onSuccess: (res) => {
        ...
        addToast({
          type: "success",
          message: `${d?.rejectedCount ?? 0}명 일괄 거절 완료`,
        });
        setSelected([]);
        invalidate();
      },
      onError: () => {
        addToast({ type: "error", message: "일괄 거절 실패" });
      },
    },
  });
  ...
  const rejectedData =
    rejectedRes?.status === 200 ? rejectedRes.data : undefined;
              ...
              <tr className="typo-c1 text-muted-foreground uppercase tracking-widest border-b border-border">
                <th className="pb-s4">
                  <input
                    type="checkbox"
                    checked={
                      selected.length === pendingList.length &&
                      pendingList.length > 0
                    }
                    onChange={() =>
                      toggleAll(
                        pendingList.map((a) => a.userId!).filter(Boolean),
                      )
                    }
            ...
            <tbody className="divide-y divide-border">
              {pendingList.map((a) => (
                <tr key={a.userId}>
                  ...
                  <td className="py-s4 typo-b2 text-muted-foreground hidden lg:table-cell max-w-[200px] truncate">
                    {a.motivation}
                  </td>
                  ...
                  <td className="py-s4 text-right">
                    {rejectingId === a.userId ? (
                      ...
                      <div className="flex gap-s2 justify-end">
                        <Button
                          size="xs"
                          onClick={() => approveOne({ id: a.userId! })}
                          disabled={isBusy}
                        >
                          승인
                        </Button>
                        <Button
                          size="xs"
                          variant="destructive"
                          onClick={() => setRejectingId(a.userId!)}
                          disabled={isBusy}
                        >
                          거절
                        </Button>
            ...
            <tbody className="divide-y divide-border">
              {rejectedList.map((a) => (
                <tr key={a.userId}>
                  ...
                  <td className="py-s4 typo-b2 text-muted-foreground">
                    {a.rejectedAt
                      ? new Date(a.rejectedAt).toLocaleDateString("ko-KR")
                      : "-"}
                  </td>
                </tr>
              ))}
