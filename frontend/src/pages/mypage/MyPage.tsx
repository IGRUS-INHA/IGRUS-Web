...
export default function MyPage() {
  ...
  const tabCounts = {
    ...
    likes: likesQuery.isLoading
      ? "-"
      : likesQuery.posts.filter((p) => !p.isDeleted).length,
    scraps: bookmarksQuery.isLoading
      ? "-"
      : bookmarksQuery.posts.filter((p) => !p.isDeleted).length,
    events: registrationsQuery.isLoading
      ? "-"
      : registrationsQuery.registrations.filter((r) => r.status !== "CANCELED")
          .length,
  };

  // 프로필 데이터 (API 응답 우선, fallback으로 auth store)
  const profileData =
    profile.data?.status === 200 ? profile.data.data : undefined;
    ...
    <div className="space-y-s8 animate-in fade-in duration-300">
      ...
      >
        <h3 className="text-xl font-bold mb-s6 flex items-center gap-s2 capitalize">
          ...
          {activeTab === "events"
            ? "신청한 행사"
            : `${activeTab === "posts" ? "게시글" : activeTab === "likes" ? "좋아요" : "스크랩"} 내역`}
        </h3>
        ...
        <div className="space-y-s4">
              ...
              {postsQuery.posts.map((post) => (
                <div
                  ...
                  onClick={() =>
                    post.boardCode &&
                    post.id &&
                    navigate(`/board/${post.boardCode}/${post.id}`)
                  }
                  className={cn(
                ...
                >
                  ...
                  <div className="text-sm font-bold text-primary cursor-pointer ml-s4 shrink-0">
                    보기
                  </div>
                </div>
              ))}
                ...
                .map((post) => (
                  <div
                    ...
                    onClick={() =>
                      post.boardCode &&
                      post.postId &&
                      navigate(`/board/${post.boardCode}/${post.postId}`)
                    }
                    className={cn(
                      "p-s6 rounded-r4 border flex justify-between items-center transition-all hover:scale-[1.01] cursor-pointer",
                      isDark
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-s2 mb-s1">
                        ...
                        />
                        <p className="typo-c1 text-primary font-bold">
                          {post.boardName ?? "스크랩한 자료"}
                        </p>
                      </div>
          ...
          {activeTab === "events" && (
            <TabContent
              ...
              isEmpty={
                registrationsQuery.registrations.filter(
                  (r) => r.status !== "CANCELED",
                ).length === 0
              }
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-s4">
                {registrationsQuery.registrations
                    ...
                    >
                      <div>
                        <h4 className="font-bold text-lg">
                          {reg.eventTitle ?? "행사명 없음"}
                        </h4>
                        <p
                          className={cn(
                            "typo-c1 mt-s1 font-bold",
                            getRegistrationStatusColor(reg.status),
                          )}
                        >
                          상태:{" "}
                          {reg.status
                        ...
                        <p className="typo-c1 text-muted-foreground mt-s2">
                          {reg.eventStartAt
                            ...
                            : "일정 미정"}
                          {reg.registeredAt
                            ? ` \u2022 신청일 ${formatDate(reg.registeredAt)}`
                            : ""}
                        </p>
                      </div>
                    </div>
