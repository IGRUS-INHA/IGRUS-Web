...
- 작성자 표시는 **학번 앞 2자리 + 이름** (예: `22 오유찬`) — 학번 전체는 비공개.
- **클릭수는 외부 비공개** — 공개 API(목록/상세)에는 포함하지 않고, 본인(`/mine`)과
  운영진 응답에만 포함.

### 수정 (재승인)

- 본인 작품은 `/my` 에서 수정 가능 (`PUT /api/projects/{id}`).
- **승인작 수정**: 라이브 버전은 그대로 유지되고, 수정본이 `project_revisions` 에
  대기(`pending`) → 운영진 승인 시 라이브에 반영(클릭수·score 유지), 반려 시 사유만 기록.
  프로젝트당 수정본 1개 — 재수정하면 덮어쓴다. 배너 제거는 미지원(교체만).
- **심사중/반려작 수정**: 바로 덮어쓰고 다시 `pending` 이 된다.
- 검수 대기 탭에는 신규 제출과 수정 요청(`update` 필드에 새 버전)이 함께 나온다.
