...
- Q: 게시글 목록 페이징 기본값 - 기본 페이지 크기는 얼마로 설정해야 하나요? → A: 20개 (균형잡힌 표준 설정)
...
- S3 이미지 업로드가 실패하면 어떻게 되는가? → 게시글 작성은 성공하고, 실패한 이미지는 건너뛰며 사용자에게 알림 표시
...
- **FR-014**: 게시글 목록 API의 기본 페이지 크기는 20개로 설정해야 한다
