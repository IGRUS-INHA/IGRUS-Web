...
- Q: 게시글에 파일 첨부 기능을 지원해야 하나요? → A: 다중 파일 허용 - 이미지 파일 첨부 가능 (최대 5개, 각 10MB)
...
- **FR-014**: 게시글에 이미지 첨부 기능을 제공해야 한다 (최대 5개, 각 10MB 제한)
...
- **PostImage** *(DB Entity)*: 게시글 이미지. 이미지 URL(S3), 표시 순서. 게시글당 최대 5개, 파일당 최대 10MB. AWS S3에 저장.
