...
- Suspended 상태에서는 본인 탈퇴(Withdrawn) 불가 (정지 해제 후에만 탈퇴 가능)
...
**게시판 목록 조회**
- 전체 게시판 목록 조회 (권한 설정, 옵션 포함)

**게시판 생성**
- 게시판 이름, 코드, 설명 입력
- 읽기/쓰기 권한 설정 (역할별)
- 옵션 설정 (익명 허용, 질문 태그 허용)
...
| GET | /boards | 게시판 관리 목록 조회 | ADMIN |
| POST | /boards | 게시판 생성 | ADMIN |
| PUT | /boards/:code | 게시판 수정 | ADMIN |
| DELETE | /boards/:code | 게시판 삭제 | ADMIN |
