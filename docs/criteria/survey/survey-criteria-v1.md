...
#### 운영 시나리오 요약 (Quick Reference)

위 표 5의 주요 시나리오를 운영자 관점에서 풀어 쓴 것이다. 2축 모델에서 **visibility(공개 여부)** 와 **responseStatus(응답 수집 여부)** 가 독립적으로 동작함을 보여준다.

| 상황 | 상태 흐름 | 비고 |
|------|----------|------|
| 설문 만들고 바로 배포 | `(U, NS)` → `publishAndOpen()` → `(P, O)` | 가장 일반적인 흐름 (F2) |
| 공개만 하고 응답은 나중에 | `(U, NS)` → `publish()` → `(P, NS)` | 응답자에게 미리보기 노출 (F4) |
| 마감일 경과로 자동 마감 | `(P, O)` → 스케줄러 → `(P, C)` | 공개 상태 유지. 결과 확인 가능 (F5) |
| 수동으로 응답 마감 | `(P, O)` → `closeResponse()` → `(P, C)` | 공개 상태 유지. 결과 확인 가능 (F1) |
| 마감된 설문 응답 재개 | `(P, C)` → `openResponse()` → `(P, O)` | 마감일 미경과 필수 (F3) |
| 끝난 설문 목록에서 숨기기 | `(P, C)` → `unpublish()` → `(U, C)` | 응답 데이터 보존 (F11) |
| 진행 중 설문 긴급 내리기 | `(P, O)` → `unpublish()` → `(U, C)` | OPEN이면 자동 마감 (INV-20) (F12) |
| 숨긴 설문 다시 공개 | `(U, C)` → `publish()` → `(P, C)` | 마감 상태로 공개됨 (F13) |
| 숨긴 설문 공개 + 응답 재개 | `(U, C)` → `publishAndOpen()` → `(P, O)` | 한 번에 공개 + 응답 시작 (F14) |
| 설문 복사 후 재사용 | 원본 복사 → `(U, NS)` → 수정 → `publishAndOpen()` → `(P, O)` | 응답 0건으로 시작 (F18) |

> **핵심**: `closeResponse()`는 응답 수집만 마감하고 공개 상태는 유지한다. `unpublish()`는 목록에서 숨기며, OPEN이면 자동 마감을 동반한다. 두 축은 독립적으로 동작한다.
