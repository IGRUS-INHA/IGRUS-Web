...
> 이 기능의 설계 결정 사항을 명시한다. 각 DECISION은 검증 기준 전반에 영향을 미치며, 본문의 불변조건과 검증 기준은 이 결정에 따라 작성되었다.

| ID | 질문 | 선택지 | 확정안 | 영향 범위 |
|----|------|--------|--------|----------|
| DECISION-01 | EventGroup과 Event의 관계 모델 | (A) Event에 `@ManyToOne` FK 추가 (N:1), (B) 중간 테이블 `event_group_events` (M:N), (C) Event에 nullable `groupId` 약한 참조 (Long, FK 없음) | **(A) 확정**: Event에 `@ManyToOne(fetch = LAZY)` + `@JoinColumn` FK 관계 설정. 동일 도메인(event 패키지) 내 관계이므로 Event→User, EventRegistration→Event 등 프로젝트 지배 패턴과 일관. DB 레벨 참조 무결성 보장. | 1절 전체, 3-1, 부록 A DB 마이그레이션 |
| DECISION-02 | 하나의 행사가 여러 그룹에 소속 가능한가 | (A) 1:N -- 행사는 최대 1개 그룹에만 소속, (B) M:N -- 행사가 여러 그룹에 소속 가능 | **(A) 확정**: 1:N 관계. 행사는 최대 1개 그룹에만 소속. DECISION-01의 `@ManyToOne` FK 방식에 의해 자연스럽게 1:N이 됨. | EGRP-INV-03, 3-3 |
| DECISION-03 | 그룹 삭제 시 소속 행사 처리 정책 | (A) 소속 행사의 `eventGroup`을 null로 변경 (연결 해제), (B) 소속 행사가 있으면 삭제 거부, (C) 그룹만 soft delete하고 행사의 FK는 유지 (orphan 허용) | **(B) 확정**: 소속 행사가 있으면 삭제 거부. 실수로 행사-그룹 연결이 끊어지는 것을 방지. 삭제하려면 먼저 모든 소속 행사를 제거해야 함. | EGRP-INV-08, 3-6 |
| DECISION-04 | 그룹 조회 API 배치 (공개/관리자) | (A) 공개 API만 (`/api/v1/event-groups`), (B) 관리자 API만 (`/api/v1/admin/event-groups`), (C) 공개 + 관리자 양쪽 | **(C) 확정**: 공개 API + 관리자 API 양쪽 제공. 일반 사용자는 PUBLISHED 행사만, 관리자는 모든 행사 포함. | 4절 전체, 5절 로그, 부록 B |
| DECISION-05 | 그룹 내 행사 정렬 기준 | (A) 행사 시작일(`eventStartAt`) 오름차순, (B) 그룹 추가 순서, (C) 별도 `displayOrder` 필드 | **(A) 확정**: `eventStartAt` 오름차순 정렬. 시간순 정렬이 가장 직관적. | EGRP-INV-07 |
| DECISION-06 | 그룹에 소속되지 않은 행사도 그룹 목록 API에서 "미분류"로 표시하는가 | (A) 미분류 행사는 별도 API/필터로 조회, (B) 그룹 목록 응답에 "미분류" 가상 그룹 포함 | **(A) 확정**: 그룹 API는 그룹만 반환. 미분류 행사(eventGroup=null)는 기존 행사 목록 API로 조회. | 3-4, 부록 B API 설계 |
| DECISION-07 | 이미 같은 그룹에 소속된 행사를 다시 추가할 때의 동작 | (A) 멱등적 성공 (200 OK, 변경 없음), (B) 충돌 거부 (409 Conflict) | **(A) 확정**: 멱등적 성공 (200 OK, 변경 없음). 기존 Storage Confirm API 멱등성 패턴과 일관. | EGRP-INV-03, 3-3, GAP-EGRP-03 |
| DECISION-08 | 그룹에 소속되지 않은 행사를 제거 시도할 때의 동작 | (A) 404 Not Found (해당 그룹에 소속된 행사가 아님), (B) 400 Bad Request (잘못된 요청) | **(A) 확정**: 404 Not Found 반환. 그룹-행사 연결을 리소스로 보면 "존재하지 않는 연결"이므로 404가 RESTful 의미론에 부합. | 3-4, GAP-EGRP-19 |
| DECISION-09 | 그룹 수정 API의 HTTP 메서드 (PUT vs PATCH) | (A) PUT (전체 수정), (B) PATCH (부분 수정) | **(A) 확정**: PUT (전체 수정). 수정 가능 필드가 `name`과 `description` 2개뿐이므로 전체 수정이 적합. `backend/CLAUDE.md`의 "PUT: 전체 수정" 원칙에 따름. | 부록 B API, 5절 로그 메시지 |
| DECISION-10 | 그룹 목록 조회의 페이지네이션 방식 | (A) `Page<T>` (총 개수 포함), (B) `Slice<T>` (다음 페이지 존재 여부만), (C) 페이지네이션 없음 (전체 반환) | **(C) 확정**: 페이지네이션 없이 전체 반환. 동아리 규모상 그룹 수가 수십 개 이내로 예상. 향후 필요 시 `Page<T>`로 전환 가능. | 3-5, 부록 B API |
| DECISION-11 | 공개 그룹 목록/상세 조회 API의 인증 요구 수준 | (A) 비인증 허용 (`permitAll()`), (B) 인증 필수 (`isAuthenticated()`) | **(B) 확정**: 인증 필수. `ApiSecurityConfig`의 `.anyRequest().authenticated()` 기본 규칙 적용. 기존 행사 상세 조회와 일관. | 4절 전체, SEC-EGRP-01~02 |
...
> 하나의 행사(Event)는 최대 1개의 그룹에만 소속될 수 있다 (1:N 관계, DECISION-02).

- **사전조건**: 행사를 그룹에 추가하는 요청
- **사후조건**: `Event.eventGroup`이 null이 아닌 행사는 다른 그룹에 추가할 수 없음
- **위반 시 예외**: `EventAlreadyInGroupException` (409 Conflict)
- **관련 코드**: **(신규 구현 필요)**
  - `EventGroupService.addEventToGroup()` -- `event.getEventGroup() != null`이면 동작 분기
  - `Event.eventGroup` -- nullable `@ManyToOne(fetch = LAZY)` FK 관계 (DECISION-01)
- **주의사항**:
  - 이미 **같은 그룹**에 소속된 행사를 다시 추가 시도하면 멱등적 성공 (200 OK, 변경 없음) (DECISION-07)
  - 이미 **다른 그룹**에 소속된 행사를 추가하려면 먼저 기존 그룹에서 제거해야 함 (409 Conflict 반환)
- **검증 방법**: 그룹 A에 소속된 행사를 그룹 B에 추가 시도 시 409 예외 발생 확인, 같은 그룹에 재추가 시 멱등적 성공 (200 OK) 확인
...
> 그룹에 소속된 행사 목록은 `eventStartAt` 오름차순으로 정렬된다 (DECISION-05).
...
### EGRP-INV-08: 소속 행사가 있는 그룹 삭제 거부

> 소속 행사가 1개 이상인 그룹은 삭제할 수 없다 (DECISION-03).

- **사전조건**: 운영자 그룹 삭제 요청 (`deleteEventGroup()`)
- **위반 시 예외**: `EventGroupNotEmptyException` (409 Conflict)
- **사후조건**:
  - 소속 행사가 0개인 그룹만 삭제 가능 → `EventGroup.deleted == true`
  - 소속 행사가 있으면 삭제 거부, DB 상태 변경 없음
- **관련 코드**: **(신규 구현 필요)**
  - `EventGroupService.deleteEventGroup()` -- 소속 행사 수 확인 후 0이 아니면 예외
  - `EventRepository.countByEventGroupAndDeletedFalse(eventGroup)` 또는 `existsByEventGroup(eventGroup)`
- **주의사항**:
  - 삭제하려면 먼저 모든 소속 행사를 그룹에서 제거해야 함 (행사 제거 API 사용)
  - 소프트 삭제된 행사(`event_deleted = true`)는 카운트에서 제외 (`@SQLRestriction`에 의해 자동 필터링)
- **검증 방법**: 소속 행사가 있는 그룹 삭제 시도 시 409 예외 발생 확인, 행사 모두 제거 후 삭제 성공 확인

### EGRP-INV-09: 행사 삭제 시 그룹 연결 해제

> 행사가 삭제(soft delete)되면, 해당 행사의 `eventGroup` 참조는 삭제 전 값을 유지하되 그룹 조회 시 해당 행사는 포함되지 않는다.
...
**HTTP 메서드**: PUT -- 전체 수정 (DECISION-09). `name`과 `description` 모든 필드를 전달하는 전체 수정 방식.
...
|---------|:---:|:---:|------|
| 정상 추가 | 활성, eventGroup=null | 활성 | 성공 (200) |
| 이미 같은 그룹 소속 | 활성, eventGroup=해당그룹 | 활성 | 멱등적 성공 (200 OK, 변경 없음) (DECISION-07) |
| 다른 그룹 소속 | 활성, eventGroup=다른그룹 | 활성 | 거부 (409, EGRP-INV-03) |
...
| 시나리오 | 기대 결과 |
|---------|----------|
| 그룹에 소속된 행사 제거 | 성공 (200), 행사의 `eventGroup = null` |
| 그룹에 소속되지 않은 행사 제거 시도 | 거부 (404 Not Found) -- 존재하지 않는 연결 (DECISION-08) |
...
| 그룹이 1개인 경우 | 1개 그룹 반환 |
| 그룹이 다수(50개)인 경우 | 전체 반환 (페이지네이션 없음, DECISION-10) |
...
| 행사 있는 그룹 삭제 | 소속 행사 3개 | 삭제 | 거부 (409, EGRP-INV-08, DECISION-03). 먼저 소속 행사를 모두 제거해야 함 |
| 이미 삭제된 그룹 재삭제 | deleted=true | 삭제 | 거부 (404, EGRP-INV-04에 의해 조회 불가) |
| 행사가 다른 그룹에도 소속된 경우 | 1:N 관계이므로 불가 (DECISION-02) | - | 해당 없음 |
...
#### 공개 API (`/api/v1/event-groups/**`) -- 인증 필수 (DECISION-04, DECISION-11)
...
| `PUT /api/v1/admin/event-groups/{groupId}` | `그룹 수정 요청 - groupId: {}, userId: {}` |
...
| EGRP-INV-08 (소속 행사 있으면 삭제 거부) | - | **누락** (신규) |
...
| GAP-EGRP-03 | 행사의 단일 그룹 소속 제약 테스트 -- 동일 그룹 재추가 시 멱등적 성공(200) 포함 (EGRP-INV-03, DECISION-07) | **높음** | 미해결 |
...
| GAP-EGRP-08 | 소속 행사가 있는 그룹 삭제 시 409 거부 테스트 + 행사 모두 제거 후 삭제 성공 테스트 (EGRP-INV-08) | **높음** | 미해결 |
...
| GAP-EGRP-15 | 소속 행사가 있는 그룹 삭제 거부 시 행사 상태 및 그룹 연결 변경 없음 확인 (부작용 없음) | **높음** | 미해결 |
...
| GAP-EGRP-19 | 그룹에 소속되지 않은 행사 제거 시 404 Not Found 응답 확인 (DECISION-08) | **중간** | 미해결 |
...
> 이 섹션은 확정된 DECISION에 따른 예상 설계이다.
...
### Event 엔티티 변경 (DECISION-01: @ManyToOne FK)

```java
// Event.java에 필드 추가
@ManyToOne(fetch = FetchType.LAZY)
@JoinColumn(name = "event_group_id")
private EventGroup eventGroup;  // nullable, FK 관계 (Event→User 패턴과 동일)
```
...
-- events 테이블에 group_id 컬럼 추가 (nullable, FK 설정)
ALTER TABLE events ADD COLUMN event_group_id BIGINT NULL;
ALTER TABLE events ADD CONSTRAINT fk_event_event_group
...
- `event_group_id` (events 테이블): `@ManyToOne` + `@JoinColumn` FK 관계 (DECISION-01). Event→User, EventRegistration→Event 등 동일 도메인 내 관계 패턴과 동일. `Event.surveyId`(크로스 도메인 약한 참조)와 달리, EventGroup은 같은 event 패키지 내이므로 FK가 적합.

---

## 부록 B: API 엔드포인트 설계 (예상)

> 이 섹션은 확정된 DECISION에 따른 예상 API 설계이다.

### 공개 API

| 메서드 | 경로 | 설명 | 인증 | 응답 코드 |
|--------|------|------|------|----------|
| `GET` | `/api/v1/event-groups` | 그룹 목록 조회 (전체 반환, 페이지네이션 없음) | 인증 필요 | 200 OK |
| `GET` | `/api/v1/event-groups/{groupId}` | 그룹 상세 조회 (PUBLISHED 행사만) | 인증 필요 | 200 OK |
...
| `PUT` | `/api/v1/admin/event-groups/{groupId}` | 그룹 수정 (PUT 전체 수정) | OPERATOR+ | 200 OK |
