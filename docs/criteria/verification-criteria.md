...
이 프로젝트는 주로 AI를 사용해 개발되었다. AI가 생성한 코드는 개별 기능은 올바르게 구현하더라도, 기능 간 상호작용이나 시스템 전체의 불변조건을 위반할 수 있다. 이 문서는 회원가입/승인/강등 도메인에서 **반드시 지켜져야 하는 규칙**을 명시하여, 코드 변경 시 검증 기준으로 사용한다.
